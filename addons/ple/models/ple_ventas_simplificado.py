...
class ple_ventas (models.Model):
    _inherit= "ple.base"
    _name = "ple.ventas_simple"

    linea = fields.One2many('ple.ventas_simple_linea', 'base_id' , string="detalle de reporte")

    @api.multi
    def imprimir(self):
        self.ensure_one()
        return self.env.ref('ple.report_ventas_14_2').report_action(self)

    @api.one
    def recargar(self):
        self.env["ple.ventas_simple_linea"].search([("period_id_linea.code", "=", self.period_id.code)]).unlink()

        self.write({'state': 'generated'})
        linea_obj = self.env['ple.ventas_simple_linea']
        docs = self.env["account.invoice"].search([("period_id","=",self.period_id.code),("type","=","out_invoice"),("state","!=","draft")])
    ...
    def exportar(self):
        ...
        file_name = "LE" + self.company_id.partner_id.vat +modificar_fecha_periodo(self.period_id.code)+ "140200"+"001"+("0" if self.sin_movimiento else "1" )+ "1" + "1.txt"
        values = {
            'name': file_name,
    ...
    def _get_txt(self):
        i=1
        txt=""
        documents = self.env['ple.ventas_simple'].search([ ['period_id.code', '=', self.period_id.code] ])
...
class ple_compras_linea(models.Model):
    _inherit = 'ple.linea'
    _name = 'ple.ventas_simple_linea'

    base_id = fields.Many2one('ple.ventas_simple')
