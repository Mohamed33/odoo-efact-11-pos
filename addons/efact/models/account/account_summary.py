...
"""
{
	...
	"resumen": {
		...
		"tipoDocEmisor": "6",
		"fechaReferente": "2018-06-08",
		"nombreEmisor": "VICTOR DANIEL MORENO LOPEZ"
	},
	"detalle": [
		{
			"serie": "B001",
...
class AccountSummaryLine(models.Model):
    ...
    account_summary_id = fields.Many2one("account.summary",string="Resumen Diario",ondelete="set null")
    ...
    cod_operacion = fields.Selection(string="Código Operación",selection=[("1","Adicionar"),("2","Modificar"),("3","Anular")])
...
class AccountSummary(models.Model):
    ...
    def write(self,vals):
        ...
        for account_invoice_id in vals.get("account_invoice_ids",[]):
            if account_invoice_id[0] == 2:
                self.env["account.invoice"].browse(account_invoice_id[1]).write({"account_summary_id":False})
            else:
    ...
    def _change_account_invoice_ids(self):
        for record in self:
            ...
            account_invoices = [{
                    ...
                    "correlativo":int(invoice.number.split("-")[1]),
                    "tipo_documento":invoice.invoice_type_code,
                    "tipo_doc_receptor":invoice.partner_id.tipo_documento,
                    ...
                    "tipo_doc_referencia":invoice.refund_invoice_id.invoice_type_code if invoice.refund_invoice_id else "",
                    "num_doc_referencia":invoice.refund_invoice_id.number if invoice.refund_invoice_id else ""
                } for invoice in account_invoices]
            
            summary_line_ids = []
            for ai in account_invoices:
                summary_line_ids.append(self.env["account.summary.line"].create(ai).id)
    ...
    def actualizar_lineas_resumen(self):
        for record in self:
            ...
            account_invoices = [{
                    ...
                    "correlativo":int(invoice.number.split("-")[1]),
                    "tipo_documento":invoice.invoice_type_code,
                    "tipo_doc_receptor":invoice.partner_id.tipo_documento if invoice.partner_id.tipo_documento in ["0","1","4","6","7","A"] else "0",
                    "numero_doc_receptor":invoice.partner_id.vat,
                    "tipo_moneda":invoice.currency_id.name,
                    "codigo_afectacion_igv":(invoice.tax_line_ids[0].tax_id.tipo_afectacion_igv.code if len(invoice.tax_line_ids)>0 else "10"),
                    "monto_total":invoice.amount_total,
    ...
    def cargar_comprobantes(self): 
        if self.cod_operacion == "1":
            domain_status_comprobante = ("estado_comprobante_electronico","in",["-",False,"0_NO_EXISTE"])
        elif self.cod_operacion in ["2","3"]:
            domain_status_comprobante = ("estado_comprobante_electronico","in",["1_ACEPTADO"])

        if not self.company_id: raise UserError("Debe seleccionar una compañía")
        ...
        nota_debito_ids = self.env["account.invoice"].search([("date_invoice","=",self.fecha_emision_documentos),
                                                                    ("state","in",["open","paid"]),
                                                                    ("invoice_type_code","=","08"),
                                                                    domain_status_comprobante])

        nota_debito_ids = [nd for nd in nota_debito_ids if nd.refund_invoice_id.invoice_type_code == "03"]
        ...
        self.account_invoice_ids = [(6,0,[account_invoice["id"] for account_invoice in account_invoices])]

        if self.summary_line_ids:
            for sl in self.summary_line_ids:
                sl.unlink()
        self.summary_line_ids = account_invoices
    ...
    def _check_cod_operacion(self):
        for record in self:
            if record.estado_emision != "B":
                raise UserError("Sólo puede cambiar el código de operación cuando el estado del comprobante posee estado 'Borrador v  '")
    ...
    def generar_identificador_resumen(self):
        for record in self:
            if not record.numero_envio:
                documents = self.env['account.summary'].search([['fecha_generacion', '=', record.fecha_generacion]])
                documents = documents.sorted(key=lambda r:r.numero_envio,reverse=True)
                ...
                if record.fecha_generacion:
                    fecha = datetime.strptime(record.fecha_generacion, "%Y-%m-%d").strftime("%Y%m%d")
                    record.identificador_resumen = "RC-"+fecha+"-"+str(record.numero_envio).zfill(3)
    """
    ...
    def generar_resumen_diario(self):
        for record in self:
            ...
            resumen_diario_json["resumen"] = {
                ...
                "tipoDocEmisor":record.company_id.partner_id.tipo_documento,
    ...
    def btn_enviar_resumen_diario(self):
        ...
        #resumen = self.env["account.summary"].search([("identificador_resumen","=",self.identificador_resumen)])
        self.generar_identificador_resumen()
        self.generar_resumen_diario()
    ...
    def enviar_resumen_diario(self):
        ...
        if response_env.ok:
            ...
            if result:
                self.estado_emision = result.get('sunat_status')
                self.estado = "enviado"
    ...
    def cron_crear_resumenes_diarios(self):
        date_order_utc = datetime.strptime(fields.Datetime.now(),"%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone("UTC"))
        date_order = date_order_utc.astimezone(timezone("America/Lima")).strftime("%Y-%m-%d")
        fecha_ayer = datetime.strptime(fields.Date.today(),'%Y-%m-%d') - timedelta(days=2)
        ...
        try:
            for company in invoices.mapped("company_id"):
                invoices_by_company = invoices.filtered(lambda inv: inv.company_id == company and re.match("^B\w{3}-\d{1,8}$", inv.move_name))
                date_invoices = list(set(invoices_by_company.mapped("date_invoice")))

                for date_invoice in date_invoices:
                    invoices_by_company_by_date = invoices_by_company.filtered(lambda inv: inv.date_invoice == date_invoice)
                    chunks = [invoices_by_company_by_date[x:x+300] for x in range(0,len(invoices_by_company_by_date),300)]
                    for invs in chunks:
                        resumen = {
...
class AccountSummaryAnularComprobante(models.TransientModel):
    ...
    account_invoice_id = fields.Many2one("account.invoice",string="Comprobante Electŕonico")
    ...
    def btn_anular_comprobante(self):
        ...
        date_order_utc = datetime.strptime(fields.Date.today(),"%Y-%m-%d").replace(tzinfo=timezone("UTC"))
        date_order = date_order_utc.astimezone(timezone("America/Lima")).strftime("%Y-%m-%d")
