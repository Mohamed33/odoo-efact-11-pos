...
from ..auth.oauth import generate_token_by_company
import requests
import os
import re
import json
...
class ModalidadTransporte(models.Model):
    _name = "efact.modalidad_transporte"
    code = fields.Char("Código")
    name = fields.Char("Descripción")

class MotivoTraslado(models.Model):
    _name = "efact.motivo_traslado"
    code = fields.Char("Código")
    name = fields.Char("Descripción")
    active = fields.Boolean("Activo",default=True)

class GuiaRemisionLine(models.Model):
    _name = "efact.guia_remision_line"
    product_id = fields.Many2one("product.product",required=True)
    uom_id = fields.Many2one("product.uom",string="UM",required=True)
    qty = fields.Float(string="Cantidad")
    guia_remision_id = fields.Many2one("efact.guia_remision",required=True,ondelete='cascade')
    ...
    def _check_description(self):
        for record in self:
            if len(record.description)<4 and len(record.description)>100:
                raise UserError("La Descripción del producto debe tener por lo menos 4 carácteres y a lo ás 100 carácteres.")
...
class ResPartner(models.Model):
    ...
    vehiculo_ids = fields.One2many("efact.vehiculo","propietario_id",string="Vehículos")
    ...
    def action_view_conductores_privados(self):
        return {
            ...
            'view_mode': 'tree,form',
            'views': [[self.env.ref("efact.view_tree_partner_conductor").id,"tree"],[self.env.ref("efact.view_form_partner_conductor").id,"form"]],
...
class Vehiculo(models.Model):
    ...
    def action_view_vehiculos_privados(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
            'views': [[self.env.ref("efact.view_tree_vehiculo").id,"tree"],[self.env.ref("efact.view_form_vehiculo").id,"form"]],
            'res_model': 'efact.vehiculo',
            'target': 'self',
...
class PopupFormSelectUbigeo(models.TransientModel):
    _name = 'efact.popup_form_seleccion_ubigeo'
    ...
    distrito_id = fields.Many2one("res.country.state",string="Distrito",required=True)
    guia_remision_id = fields.Many2one("efact.guia_remision",required=True)
    tipo_lugar = fields.Selection(selection=[("partida","Partida"),("llegada","Llegada")],required=True)
    ...
    def set_ubigeo(self):
        ...
        if record.tipo_lugar == "partida":
            ...
            record.guia_remision_id.write({"lugar_llegada_ubigeo" : record.ubigeo})

class GuiaRemision(models.Model):
    _name = "efact.guia_remision"
    _rec_name = "numero"
    ...
    company_partner_id = fields.Many2one("res.partner",related="company_id.partner_id",readonly=True)
    
    #SERIE Y CORRELATIVO
    journal_id = fields.Many2one("account.journal",string = "Serie",states={'validado': [('readonly',True)]})
    correlativo = fields.Integer(string = "Correlativo",copy=False)
    serie = fields.Char("Prefijo",related="journal_id.code",copy=False)
    numero = fields.Char("Número",default="Guía de Remisión - Borrador",copy=False)
    name = fields.Char("Nombre",index=True,states={'validado': [('readonly',True)]},copy=False)
    
    request_json = fields.Text("Petición JSON", states={'validado': [('readonly',True)]},copy=False)
    response_json = fields.Text("Respuesta JSON", states={'validado': [('readonly',True)]},copy=False)

    @api.model
    def default_get(self, flds):
        res = super(GuiaRemision, self).default_get(flds)
        journals = self.env["account.journal"].search([("invoice_type_code_id","=","09")])
        if len(journals)>0:
            res.update({
    ...
    #DESTINATARIO
    motivo_traslado = fields.Selection(selection="_list_motivo_traslado",string="Motivo de Traslado",states={'validado': [('readonly',True)]})

    def _list_motivo_traslado(self):
        motivo_traslado_objs = self.env["efact.motivo_traslado"].search([])
        return [(mt.code,mt.name) for mt in motivo_traslado_objs]

    destinatario_partner_id = fields.Many2one("res.partner",string="Destinatario",states={'validado': [('readonly',True)]})
    destinatario_tipo_documento_identidad = fields.Selection(string="Tipo de Documento",related="destinatario_partner_id.tipo_documento")
    ...
    destinatario_numero_documento_identidad = fields.Char(string="Número de Documento",related="destinatario_partner_id.vat")
    destinatario_direccion = fields.Char(string="Dirección",related="destinatario_partner_id.street")
    destinatario_ubigeo = fields.Char(string="Ubigeo",related="destinatario_partner_id.zip")
    ...
    proveedor_tipo_documento_identidad = fields.Selection(string="Tipo de Documento",related="proveedor_partner_id.tipo_documento")
    ...
    proveedor_numero_documento_identidad = fields.Char(string="Número de Documento",related="proveedor_partner_id.vat")
    proveedor_direccion = fields.Char(string="Dirección",related="proveedor_partner_id.street")
    proveedor_ubigeo = fields.Char(string="Ubigeo",related="proveedor_partner_id.zip")
    ...
    documento_asociado = fields.Selection(selection=[("comprobante_pago","Comprobante de Pago"),
                                                    ("movimiento_stock","Movimiento de Stock")],
                                            string="Documento Asociado",default="movimiento_stock")

    comprobante_pago_ids = fields.Many2many("account.invoice",string="Comprobante de Pagos")

    movimiento_stock_ids = fields.Many2many("stock.picking",string="Movimientos",states={'validado': [('readonly',True)]})
            ...
            if record.documento_asociado == "movimiento_stock":
                ...
                for line in guia_remision_lines:
                    if line["product_id"] not in guia_remision_lines_temp:
                        ...
                        guia_remision_lines_temp[line["product_id"]]["qty"] += line["qty"]
                ...
                for comprobante_pago in record.comprobante_pago_ids:
                    for invoice_line in comprobante_pago.invoice_line_ids:
                        guia_remision_lines += [{
                                                ...
                                                "qty":inv_line.quantity*inv_line.uom_id.factor_inv/inv_line.product_id.uom_id.factor_inv,
    ...
    #ENVÍO  
    fecha_emision = fields.Date(string="Fecha de Emisión",states={'validado': [('readonly',True)]})
    fecha_inicio_traslado = fields.Date(string="Fecha inicio de traslado",states={'validado': [('readonly',True)]})
    #peso_bruto_total_uom_id = fields.Many2one("product.uom",string="UM",default="_default_peso_bruto_total_uom")
   

    peso_bruto_total = fields.Float(string="Peso Bruto Total (KGM)",states={'validado': [('readonly',True)]})
    ...
    def _list_modalidad_transporte(self):
        modalidad_transporte_objs = self.env["efact.modalidad_transporte"].search([])
        return [(mt.code,mt.name) for mt in modalidad_transporte_objs]


    numero_bultos = fields.Integer(string="Número de Bultos",states={'validado': [('readonly',True)]})
    ...
    direccion_llegada_id = fields.Many2one("res.partner" ,states={'validado': [('readonly',True)]})
    ...
    #DETALLE DE ENVÍO
    guia_remision_line_ids = fields.One2many("efact.guia_remision_line","guia_remision_id",
                                            string="Detalle de líneas",
                                            ondelete='cascade',states={'validado': [('readonly',True)]})

    #TRANSPORTE PRIVADO
    conductor_privado_partner_id = fields.Many2one("res.partner",string="Conductor",states={'validado': [('readonly',True)]})
    vehiculo_privado_id = fields. Many2one("efact.vehiculo",string="Vehículo",states={'validado': [('readonly',True)]})
    
    #TRANSPORTE PÚBLICO
    transporte_partner_id = fields.Many2one("res.partner",string="Empresa Transportista",states={'validado': [('readonly',True)]})
    conductor_publico_id = fields.Many2one("res.partner",string="Conductor",states={'validado': [('readonly',True)]})
    vehiculo_publico_id = fields.Many2one("efact.vehiculo",string="Vehículo",states={'validado': [('readonly',True)]})
    ...
    def set_view_lugar_partida_ubigeo(self):
        ...
        return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                "views": [[self.env.ref("efact.view_popup_form_seleccion_ubigeo").id,"form"]],
                'res_model': 'efact.popup_form_seleccion_ubigeo',
                'target': 'new',
                'context': {
    ...
    def validar_datos_compania(self):
        ...
        if not self.company_id.partner_id.vat:
            ...
            errors.append("* El RUC de la empresa emisora no tiene el formato de RUC.")
    ...
    def validar_datos_destinatario(self):
        ...
        if not partner.tipo_documento:
            errors.append("* El tipo de Documento del destinatario es obligatorio")
        else:
            if partner.tipo_documento == "6":
                if not partner.vat:
                    errors.append("* El Número de documento del destinatario esta vacío.")
                elif not patron_ruc.match(partner.vat):
                    errors.append("* El Número de documento del destinatario tiene un RUC inválido.")
            elif partner.tipo_documento == "1":
                if not partner.vat:
                    errors.append("* El Número de documento del destinatario esta vacío.")
                elif not patron_dni.match(partner.vat):
                    errors.append("* El Número de documento del destinatario tiene un DNI inválido.")
            else:
                errors.append("* El tipo de Documento del Destinatario no es válido, seleccion un RUC o DNI ")
            

        if not partner.name:
            errors.append("* El nombre o razón social del destinatario esta vacío.")
        elif len(partner.name)<4:
            errors.append("* El nombre o razón social del destinatario debe poseer más de 4 carácteres.")
        return errors
    ...
    def validar_motivo_traslado(self):
        ...
        if self.motivo_traslado == "08" and self.numero_bultos==0:
            errors.append("* El número de bulltos o Pallets es mayor a 0 sólo cuando es Importación.")
        elif self.motivo_traslado == "08" and self.numero_bultos<0:
            errors.append("* El número de bulltos o Pallets no puede ser negativo.")


        if not motivoTraslado:
            errors.append("* El Motivo de Traslado es Obligatorio.")
        motivo_traslado_activos = self.env["efact.motivo_traslado"].sudo().search([("active","=",True)])
        
        if motivoTraslado not in [ mt.code for mt in motivo_traslado_activos]:
            errors.append("* El Motivo de Traslado seleccionado no existe o no esta habilitado. Consulte con el Administrador")
        return errors
    ...
    def validar_datos_envio(self):
        ...
        if type(self.peso_bruto_total) != float:
            errors.append("* El tipo de dato del Peso Total es {} y debería ser Flotante".format(type(self.peso_bruto_total)))
    ...
    def validar_ubigeo(self,ubigeo):
        ubigeo_objs = self.env["res.country.state"].sudo().search([("code","=",ubigeo)])
    ...
    def validar_lugar_partida(self):
        ...
        if not self.lugar_partida_direccion:
            errors.append("* La dirección del lugar de partida es obligatorio.")
        elif len(self.lugar_partida_direccion)<6:
            errors.append("* La dirección del lugar de partida tiene como mínimo 6 carácteres.")
    ...
    def validar_lugar_llegada(self):
        ...
        if not self.lugar_llegada_direccion:
            errors.append("* La dirección del lugar de llegada es obligatorio.")
        elif len(self.lugar_llegada_direccion)<6:
            errors.append("* La dirección del lugar de llegada tiene como mínimo 6 carácteres.")
    ...
    def validar_guia_remision_lineas(self):
        ...
        if len(self.guia_remision_line_ids)==0:
            errors.append("* La guía debe tener al menos un elemento en el detalle de envío.")

        for guia_remision_line in self.guia_remision_line_ids:
            if not guia_remision_line.product_id:
                errors.append("* El producto de una de las líneas del detalle está vacío.")
            
            if not guia_remision_line.description:
                errors.append("* La descripción de una de las líneas del detalle está vacío.")
            elif len(guia_remision_line.description)<4 or len(guia_remision_line.description)>250:
                errors.append("* La longitud de la descripción de un producto debe poseer una longitud mayor a 4 y menor a 250 carácteres.")
            
            if not guia_remision_line.uom_id:
                errors.append("* La unidad de medida del producto [{}] esta vacía".format(guia_remision_line.description))
            elif not guia_remision_line.uom_id.code:
                errors.append("* La unidad de medida del producto [{}] no tiene un código asociado. Comuníquese con su Administrador del Sistema.".format(guia_remision_line.description))
            elif guia_remision_line.uom_id.code not in codigo_unidades_de_medida:
                errors.append("* La unidad de medida [{}] del producto [{}] no esta permitido. Comuníquese con su Administrador del Sistema.".format(guia_remision_line.uom_id.code,guia_remision_line.description))

            if guia_remision_line.qty == 0:
                errors.append("La cantidad del producto [{}], debe ser mayor a 0".format(guia_remision_line.description))
    ...
    def validar_transporte(self):
        ...
        if not self.modalidad_transporte:
            errors.append("* La modalidad de transporte no ha sido seleccionado.")
        elif self.modalidad_transporte not in ["01","02"]:
            errors.append("* La modalidad de transporte seleccionado es incorrecto. Las modalidades de transporte permitidos son 01 - Transporte Público y 02 - Transporte Privado")

        if self.modalidad_transporte == "01":
            if not self.transporte_partner_id:
                errors.append("* Debe seleccionar una Empresa de Transporte público.")
            else:
                if not self.transporte_partner_id.name:
                    errors.append("* La empresa de transporte seleccionado no tiene Nombre o Razón Social.")
                elif len(self.transporte_partner_id.name)<4:
                    errors.append("* El nombre de la empresa de transporte seleccionada debe tener como mínimo 4 carácteres.")
                
                if not self.transporte_partner_id.vat:
                    errors.append("* La empresa de transporte seleccionado no tiene documento.")
                elif not patron_ruc.match(self.transporte_partner_id.vat):
                    errors.append("* El RUC de la empresa de transporte seleccionada tiene un formato incorrecto.")

                if not self.transporte_partner_id.tipo_documento:
                    errors.append("* La empresa de transporte seleccionado no tiene tipo de documento.")
                elif not self.transporte_partner_id.tipo_documento == "6":
                    errors.append("* El tipo de documento de la empresa de transporte seleccionada debe ser de tipo 'RUC'")

                if not self.fecha_inicio_traslado:
                    errors.append("* La fecha de inicio de traslado es obligatorio.")
                """
            ...
            if not self.conductor_privado_partner_id:
                ...
                if not self.conductor_privado_partner_id.name:
                    errors.append("* El conductor privado seleccionado no tiene Nombre.")
                elif len(self.conductor_privado_partner_id.name)<4:
                    errors.append("* El nombre del conductor privado seleccionado debe tener más de 4 carácteres")
                
                if not self.conductor_privado_partner_id.vat:
                    errors.append("* El conductor privado seleccionado no tiene documento.")
                if not self.conductor_privado_partner_id.tipo_documento:
                    errors.append("* La conductor privado seleccionado no tiene tipo de documento.")
            ...
            if not self.vehiculo_privado_id:
                ...
                errors.append("* El Vehículo pivado seleccionado no tiene Número de Placa")
    ...
    def generar_comprobante_json(self):
        if not self.name:
            ...
            if not re.match('T\\d{3}-\\d{1,8}', str(self.name)):
                raise UserError("El codigo no tiene el formato correcto: " + str(self.name))
            else:
        ...
        motivo_traslado_id = self.env["efact.motivo_traslado"].sudo().search([('code','=',self.motivo_traslado)])
        
        documento = {
            "serie":serie,
            "correlativo":correlativo,
            "nombreEmisor":company.name.strip(),
            "tipoDocEmisor":company.tipo_documento,
            "numDocEmisor":company.vat,
            "tipoDocReceptor":destinatario.tipo_documento,
        ...
        if self.modalidad_transporte == '01':
            transporte = {
                        ...
                        "tipoDocTransportista": self.transporte_partner_id.tipo_documento,
                        "fechaInicioTraslado": self.fecha_inicio_traslado,
            ...
            if self.vehiculo_publico_id:
                if self.vehiculo_publico_id.numero_placa:
                    transporte.update({"placaVehiculo":self.vehiculo_publico_id.numero_placa})
            if self.conductor_publico_id:
                if self.conductor_publico_id.vat:
                    transporte.update({"numDocConductor":self.conductor_publico_id.vat})
                if self.conductor_publico_id.tipo_documento:
                    transporte.update({"tipoDocConductor":self.conductor_publico_id.tipo_documento})
        ...
        #Transporte Privado
            ...
            transportes.append({
                        ...
                        "tipoDocConductor": self.conductor_privado_partner_id.tipo_documento,
                        "fechaInicioTraslado": self.fecha_inicio_traslado,
    ...
    def validar_comprobante(self):
        if self.name == "" or self.name == False:
            ...
            next_number = self.journal_id.sequence_number_next
            numero = serie + "-" + str(next_number).zfill(8)
            if len(self.env["efact.guia_remision"].search([("numero","=",numero),("state",'=','validado')]))>0:
            ...
            if len(self.env["efact.guia_remision"].search([("numero","=",self.name),("state",'=','validado')]))>0:
                raise UserError("El documento de guía de remisión ya existe.")
    ...
    def unlink(self):
        ...
        if len(l)>0:
            raise UserError("No es posible eliminar una guía de remisión Validada o que tenga un número asignado. [{}]".format(",".join(l)))
    ...
    def cron_enviar_comprobante(self,cantidad):
        guia_remision_ids = self.env["efact.guia_remision"].search([("estado_emision","in",["P",False]),("request_json","!=",False)],limit=cantidad)
    ...
    def btn_enviar_comprobante(self):
        for record in self:
            if record.estado_emision in ["P",False] and record.request_json:
                if len(self) == 1:
    ...
    def enviar_comprobante(self):
        ...
        try:
            r = requests.post(self.company_id.endpoint, headers=headers,data=data)
            ...
            if r.status_code == 200:
                ...
                if "result" in response:
                    if "errors" not in response["result"]:
                        result = response["result"]
                        if "request_id" in result:
                            log_status.update({"api_request_id":result["request_id"]})
                        if "digest_value" in result:
                            self.digest_value = result["digest_value"]
                            log_status.update({"digest_value":result["digest_value"]})
                        if "response_xml" in result:
                            log_status.update({"response_xml":result["response_xml"]})
                        if "response_content_xml" in result:
                            log_status.update({"content_xml":result["response_content_xml"]})
                        if "sunat_status" in result:
                            self.estado_emision = result["sunat_status"]
                            log_status.update({"status":result["sunat_status"]})
