...
class WizardComprobantesReport(models.TransientModel):
    ...
    def btn_generate_xlsx(self):
        report_obj = self.env.ref("report_comprobantes_efact.comprobantes_report_xlsx")
    ...
    def btn_reporte_por_proveedor(self):
        report_obj = self.env.ref("report_comprobantes_efact.compr_report_xlsx_prov")
...
class ComprobanteXLSXporProveedor(models.AbstractModel):
    _name = 'report.report_comprobantes_efact.compr_report_xlsx_prov'
    ...
    def generate_xlsx_report(self, workbook, data, records):
        ...
        domain = [['invoice_id.state','in',['open','paid']],['invoice_id.invoice_type_code','in',['00','01','03','07']],['supplier_id','=',supplier_id]]
        if fecha_fin:
            domain.append(["invoice_id.date_invoice","<=",fecha_fin])
        if fecha_inicio:
            domain.append(["invoice_id.date_invoice",">=",fecha_inicio])
        ...
        def row_line(inv_line):
            ...
            FECHA = datetime.strptime(inv_line.invoice_id.date_invoice,"%Y-%m-%d").strftime("%d/%m/%Y") if inv_line.invoice_id.date_invoice else "-"
        ...
        df_states = pd.DataFrame(invoice_lines,columns=["VENDEDOR",
                                                        ...
                                                        "TOTAL",
                                                        "PROVEEDOR"])
        df_states.to_excel(workbook, sheet_name=self.env["res.partner"].browse(supplier_id).name)

class ComprobantesXlsx(models.AbstractModel):
    _name = 'report.report_comprobantes_efact.comprobantes_report_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    ...
    def generate_xlsx_report(self, workbook, data, records):
        ...
        if fecha_fin:
            domain.append(["date_invoice","<=",fecha_fin])
        if fecha_inicio:
            domain.append(["date_invoice",">=",fecha_inicio])
        ...
        def get_serie_correlativo(numero):
            ...
            try:
                ...
                raise UserError("La serie o Correlativo del comprobante {} no es correcto".format(numero))
        ...
        def row_comp(comp):
            ...
            if comp.refund_invoice_id:
                ...
                fecha_emision_rec = comp.refund_invoice_id.date_invoice
            else:
                numero_rec,serie_rec,correlativo_rec,tipo_comp_rec,fecha_emision_rec= "","","","",""

            row = {
                "Estado":comp.state,
                "Fecha de emisión":comp.date_invoice,
            ...
            while cnt<tam:
                ...
                if correlativo_actual != df.iloc[cnt]["Correlativo Comp."]:
                    ...
                    if len(inv)==0:
                        ...
                        row_d.update({"Observacion":"Este registro se encuentra en esta fecha {}".format(inv[0].date_invoice)})
                        df = df.append(row_d,ignore_index=True)
        ...
        df = pd.DataFrame(df.values,columns=["Estado",
                                            ...
                                            'Vendedor',
                                            'Observacion'])
        df_states = df.sort_values(by=["Tipo de comprobante","Serie Comp.","Correlativo Comp."])
        """
        while cnt<tam:
                    ...
                    if  correlativo_actual != df_states.loc[cnt]["Correlativo Comp."]:
                        ...
                        if len(inv) == 0:
                            ...
                            row_d = {"Estado":"",
                                    ...
                                    'Anulación de comprobante':"",
                                    'Vendedor':"",
                                    'Observacion':"Este registro se encuentra en esta fecha {}".format(inv[0].date_invoice)}
                            #df_states = pd.DataFrame(np.insert(df_states.values, cnt, values=row_d, axis=0))
                            df_states = df_states.append(row_d,ignore_index=True)
        ...
        for row in df_states.to_dict(orient="records"):
            ...
            if row["Observacion"] or row["Estado comprobante electrónico"] != "1_ACEPTADO" and patron_comprobante.match(serie+"-"+numero):
                ...
                fecha = datetime.strptime(row["Fecha de emisión"],"%Y-%m-%d").strftime("%d-%m-%y") if row["Fecha de emisión"] else ""
                fechavcto = datetime.strptime(row["Fecha de Vencimiento"],"%Y-%m-%d").strftime("%d-%m-%y") if row["Fecha de Vencimiento"] else ""
                ...
                valvtae = "0"
                valvtai = "0"
                valvtaa = str(round(row.get("Monto Gravado",0),2)*(-1 if tipo=='07' else 1))
                igv = str(round(row.get("IGV 18% Venta",0),2)*(-1 if tipo=='07' else 1))
                total = str(round(row.get("Monto total",0),2)*(-1 if tipo=='07' else 1))
                fecharef = datetime.strptime(row.get("Fecha de emisión Comp. Rec.",""),"%Y-%m-%d").strftime("%d-%m-%Y") if row.get("Fecha de emisión Comp. Rec.") else ""
                tiporef = row.get("Tipo de comprobante a Rec.","") or ""
                serieref = row.get("Serie Comp. Rec.","") or ""
                numeroref = str(row.get("Correlativo Comp. Rec.","")).zfill(7) if row.get("Correlativo Comp. Rec.","") else ""
