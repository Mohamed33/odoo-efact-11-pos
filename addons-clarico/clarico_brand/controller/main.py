...
class claricoBrand(claricoShop):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if response.qcontext.get('selected_products', False):
            products = response.qcontext.get('selected_products')
            brand_ids = []
            for prod in products:
                if prod.brand_ept_id and prod.brand_ept_id.is_website_publish == True and prod.brand_ept_id.id not in brand_ids:
